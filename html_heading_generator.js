...
const headingGenerator = (title, headingType, statement, pType) => {
    return `                                               
      <h${headingType}>${title}</h${headingType}>
			<p${pType}>${statement}</p${pType}>
    `
  }

console.log(headingGenerator('Welcome Baby Yoda fans!', 1, 'Baby Yoda is the BEST!', 1));
